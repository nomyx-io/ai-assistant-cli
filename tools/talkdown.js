...
create_util("logOutput", "Logs a message to the console.", \`Console log the provided message.\`);
    ...
    tools: {
        set_logic: async ({ logic }) => {
            state.talkdown.logic = logic;
            return `Pseudocode logic set to: ${logic}`;
        },
        create_process: async ({ name, description, steps }) => {
        ...
        create_directive: async ({ name, description, parameters, actions }) => {
        ...
        create_function: async ({ name, description, inputs, outputs, logic }) => {
        ...
        create_util: async ({ name, description, logic }) => {
...
function compileDirectory(dirPath) {
    ...
    for (const entry of entries) {
        if (entry.isDirectory()) {
            items = items.concat(compileDirectory(path.join(dirPath, entry.name)));
        } else if (entry.isFile() && entry.name.endsWith('.md')) {
            const itemData = readAndParseMD(path.join(dirPath, entry.name));
            items.push({
                name: itemData.title || path.basename(entry.name, '.md'),
...
module.exports = {
    ...
    tools: {
        compile_talkdown_code: async ({ path }) => {
            try {
