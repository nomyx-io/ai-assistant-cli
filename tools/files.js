...
module.exports = {
    ...
    tools: {
        file: async function ({ operation, path, match, data, position, target }, run) {
            try {
        ...
        files: async function ({ operations }, run) {
            try {
                ...
                return `Successfully executed batch operations on file at path ${p}`;
            } catch (error) {
                return `Error: ${error.message}`
            }
        },
        get_file_tree: async ({ value, n }, run) => {
